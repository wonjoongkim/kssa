...
import { useInsertFAQMutation } from '../../hooks/api/BoardManagement/BoardManagement';
...
export const FaqRegister = (props) => {
    const { confirm } = Modal;
    ...
    // 공지사항 등록
    const [InsertFAQApi] = useInsertFAQMutation();
    const InsertFAQ_ApiCall = async () => {
        ...
        const InsertFAQResponse = await InsertFAQApi(formData);
        InsertFAQResponse?.data?.RET_CODE === '0100'
            ? Modal.success({
                  content: '등록 완료',
                  style: { top: 320 },
                  onOk() {
