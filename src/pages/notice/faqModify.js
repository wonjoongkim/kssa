...
import { useSelectFAQMutation, useUpdateFAQMutation, useDeleteFileMutation } from '../../hooks/api/BoardManagement/BoardManagement';
...
export const FaqModify = (props) => {
    ...
    const SelectFAQ_ApiCall = async () => {
        ...
        editorRef.current?.getInstance().setMarkdown(SelectFAQResponse?.data?.RET_DATA.contents);
        setItemContainer(SelectFAQResponse?.data?.RET_DATA);
        setUploadedFiles(SelectFAQResponse?.data?.RET_DATA.fileList);
    };

    // 공지사항 수정
    const [UpdateFAQApi] = useUpdateFAQMutation();
    const UpdateFAQ_ApiCall = async () => {
